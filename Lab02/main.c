...
int main(int argc, char *argv[]) {
	...
	struct arg_lit *yOpt  = arg_lit0("y", "yopt", "                for lab 2 task 1");
	...
	void *argTable[] = {
		...
		shellOpt, benOpt, yOpt, rstOpt, dumpOpt, helpOpt, eepromOpt, backupOpt, endOpt
	};
	...
	// newly added 'y' Option -----------------------------
	if ( yOpt->count ) {
		uint32 count = 1;
		uint8* buffer1;
		buffer1 =malloc(sizeof(uint8));
		uint8* data;
		data=malloc(sizeof(uint8));
		for (int i = 0; i <= 128; ++i){
			uint8 chan = (uint8)i;
			fStatus=flReadChannel(handle,chan,count,buffer1,&error);
			int new = i + (int)*buffer1;
			printf("%p\n",*buffer1 );
			new=new%128;
			*data = (uint8) new;
			int j=i+1;
			chan = (uint8) j;
			fStatus=flWriteChannel(handle,chan,count,data,&error);
			sleep(1); 
		}
	}
